import type { SVGProps } from "react";

// react-bootstrap-icons não tem ícone de barco/navio/âncora — mesmo SVG
// custom do warren/Portal (src/Components/Icons/ShipIcon.tsx: casco + mastro
// + vela), no mesmo estilo monocromático (16x16, fill currentColor) dos
// demais ícones do sidebar, pra bater visualmente com o item "Navio".
export function ShipIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="16"
      height="16"
      fill="currentColor"
      viewBox="0 0 16 16"
      {...props}
    >
      <path d="M7.5 1.5a.5.5 0 0 1 .5.5v1h.5a.5.5 0 0 1 .4.2l3 4a.5.5 0 0 1-.4.8H8v2h-1V8H4a.5.5 0 0 1-.4-.8l3-4a.5.5 0 0 1 .4-.2h.5V2a.5.5 0 0 1 .5-.5z" />
      <path d="M1.05 10.5h13.9l-1.62 3.9a1 1 0 0 1-.92.6H3.59a1 1 0 0 1-.92-.6L1.05 10.5z" />
    </svg>
  );
}
