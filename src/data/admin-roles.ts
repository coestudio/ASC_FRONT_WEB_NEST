 ...
 * (`src/api/generated/static/internalRoleOptions.ts`, gerado por `just map` —
 ...
 */
import { internalRoleOptions } from "@/api/generated/static/internalRoleOptions";

export type AdminRoleDescription = {
  /** Valor do enum `InternalRole` no Core (número). */
  role: number;
  /** Nome da role — mesmo texto do snapshot `internalRoleOptions` (pt-BR). */
  name: string;
...
export const ADMIN_ROLES: AdminRoleDescription[] = internalRoleOptions.map((opt) => ({
  role: Number(opt.value),
  name: opt.name["pt-BR"] ?? String(opt.value),
